...
# ==========================================================================
	...
	library(colorout) # colorize code in terminal
...
### subset invalid birthdays: overly frequent january 1 dob & unreasonable years
	...
	pha_bad_dob <- data.frame(table(ymd(pha$dob))) %>%
				   ...
				   filter(year(dob) > 2018 |
						  year(dob) < 1900 |
						  Freq >= 64) %>% # about the freq of invalid jan 1 dob
				   arrange(desc(dob)) %>%
				   pull(1)
	...
	hmis_bad_dob <- data.frame(table(ymd(hmis$DOB))) %>%
					...
					filter(year(dob) > 2018 |
						   year(dob) < 1900 |
						   Freq >= 81) %>% # about the freq of invalid jan 1 dob
					arrange(desc(dob)) %>%
					pull(1)
